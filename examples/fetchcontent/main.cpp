/**
 * unified_ml — basic usage smoke test
 *
 * This file exercises the actual public API of unified_ml.
 * All types, method names, and signatures are taken directly from the headers.
 *
 * Build with CMake (after install):
 * cmake -S . -B build -DCMAKE_PREFIX_PATH=/path/to/unified_ml
 * cmake --build build && ./build/example
 *
 * Build with pkg-config:
 * g++ main.cpp $(pkg-config --cflags --libs unified_ml) -o example
 */
#include <unified_ml>
...
int main() {
  ...
  section("PCA");
  {
    // core::Matrix must be constructed via Matrix(rows, cols) or from_rows
  ...
  // 3. Autograd (Tensor constructed from std::vector<double>)   
  section("Autograd");
  {
    // Tensor(std::vector<double>, shape, requires_grad)
  ...
  {
    // RF uses rf::Dataset, not raw vectors
    ...
    // Constructor takes RandomForestParams
  ...
  section("DBSCAN");
  {
    // DBSCAN::fit returns std::vector<int> labels
